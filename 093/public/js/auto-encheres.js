        ...
        loginBtn.addEventListener('click', () => {
            ...
            //     // On s'assure que la réponse a bien dans sa structure l'objet data.success à vrai
                ...
                // }
                    ...
                    if (data.status === 401 && data.action === 'cnx' && data.connected === false) {
                        markLoginError();
                    } else if (data.status === 200 && data.action === 'cnx' && data.connected === true) {
            ...
            if (login.value.length === 0 || password.value.length === 0 || nom.value.length === 0 || prenom.value.length === 0) {
                ...
                fetch('/', {
                    ...
                    body: JSON.stringify({
                        ...
                        "action": "register",
                        "username": window.btoa(login.value),
                        "hash": window.btoa(password_hash(password.value)),
